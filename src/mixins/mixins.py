...
def CheckBlk(request, user1, user2):
  ...
  try:
    try:
      test = BlockUser.objects.get(blocker = user1).blocked.get(blocked_u__blocked=user2)
      messages.warning(request, "This user is no longer accessible")
      return redirect("MessageListViewAll")
    except:
      test = BlockUser.objects.get(blocker = user2).blocked.get(blocked_u__blocked=user1)
      messages.warning(request, "This user is no longer accessible")
      return redirect("MessageListViewAll")
  except:
    pass
